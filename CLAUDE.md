...
- 自动生成的规则集为 `chinaIP.ip`、`telegram.ip`、`LAN.classical` 和 `ads.domain`；不要直接手工维护其内容。
...
bash -n dev/D-IP.sh dev/download-yq.sh dev/git-push.sh

# 从上游重新生成四个自动产物；需要网络和 curl，会改写目标文件
bash dev/D-IP.sh
git diff -- chinaIP.ip telegram.ip LAN.classical ads.domain
```
...
1. 创建临时目录，并通过 `dev/download-yq.sh` 下载临时使用的 `yq`。
2. 下载中国 IPv4、Telegram CIDR、LAN IP、LAN 域名、AWAvenue 广告规则和 anti-AD 域名列表。
3. 使用 `yq` 从 AWAvenue YAML 的 `payload` 提取域名，并追加 anti-AD 列表；合并 LAN IP 与 LAN 域名数据。
4. 全部下载及解析成功后，才以 `mv` 原子替换 `chinaIP.ip`、`telegram.ip`、`LAN.classical` 与 `ads.domain`。

因此，调整自动规则应修改生成逻辑或其上游输入后重跑脚本，而不是编辑生成结果。脚本失败时，四个仓库内目标文件不会在中途被覆盖。
...
`dev/git-push.sh` 仅处理 `telegram.ip`、`chinaIP.ip`、`LAN.classical`、`ads.domain`：它暂存这些文件、生成 GitHub Step Summary、在存在变更时提交并推送。修改该脚本或工作流时，只做静态语法检查与限定 diff 核对；不要触发真实 push 来验证。

## 修改边界

- 修改静态规则：只编辑指定规则文件，并用限定路径的 `git diff` 核对语法、条目及无关改动。
- 修改生成链路：运行 `bash dev/D-IP.sh` 后，仅检查四个自动产物的差异。
